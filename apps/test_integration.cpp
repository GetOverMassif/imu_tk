...
int main(int argc, char** argv)
{
    ...
    importAsciiData( argv[1], acc_data, gyro_data, mag_data, 
                    imu_tk::TIMESTAMP_UNIT_SEC, imu_tk::DATASET_SPACE_SEPARATED);
    ...
    for(int i = 3000; i < gyro_data.size() - 1; i++)
    {
        ...
        // std::cout << "t1 = " << gyro_data[i].timestamp() << ", ";
        // std::cout << "t2 = " << gyro_data[i + 1].timestamp() << std::endl;

        // std::cout << "dt = " << dt << ", ";
        // std::cout << "v1 = " << gyro_data[i] << ", ";
        // std::cout << "v2 = " << gyro_data[i + 1] << std::endl;
        // std::cout << "quat = " << quat[0] << ", " << quat[1] << ", "
